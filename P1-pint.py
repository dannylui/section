...
def Elastic_Section_Prop(Geom_Input, n: int = 8):
    ...
    Total = Section.sum()
    Centroid = Total['AY'] / Total['Area']  * units.inch
...
def calc_Ec(fc, K1=1, wc=0.150):
    ...
    return 120000 * K1 * wc**2 * (fc/units.ksi)**0.33 * units.ksi
...
def check_compactness(Geom_Input):
    ...
    if Geom_Input['fy_tf'] <= 70 * units.ksi and Geom_Input['fy_bf'] <= 70 * units.ksi:
...
Input = {'fyw':50 * units.ksi, 'fy_tf':50 * units.ksi, 'fy_bf':50 * units.ksi, 'E':29000 * units.ksi,
         'fc':4 * units.ksi,
         'Long stiffener':'no', 'Trans stiffener': 'yes',      # Should be yes/no only 
         'b_slab':114 * units.inch, 't_slab':9 * units.inch, 't_haunch':4 * units.inch, 
         'b_tf':16 * units.inch, 't_tf':1.0 * units.inch, 
         'D_web':69 * units.inch, 't_web':0.5 * units.inch, 
         'b_bf':18 * units.inch, 't_bf':1.75 * units.inch}

# Forces at section from analysis
Forces = {'M_dc1': 2202*12 * units.ft_lb, 'M_dc2': 335*12 * units.ft_lb,'M_dw': 322*12 * units.ft_lb,'M_LL': 12222 * units.ft_lb}
